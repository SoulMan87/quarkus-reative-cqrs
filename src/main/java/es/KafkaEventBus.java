...
public class KafkaEventBus implements EventBus {
    ...
    public Uni<Void> publish(List<Event> events) {
        final byte[] eventBytes = SerializerUtils.serializeToJsonBytes (events.toArray (new Event[]{}));
        final ProducerRecord<String, byte[]> record = new ProducerRecord<> (eventStoreTopic, eventBytes);
        ...
        return kafkaClientService.<String, byte[]>getProducer ("eventstore-out")
                ...
                .onFailure ().retry ().withBackOff (Duration.of (BACKOFF_TIMEOUT, ChronoUnit.MILLIS))
                .atMost (RETRY_COUNT)
                .onItem ().invoke (msg -> logger.infof ("publish key: %s, value: %s",
